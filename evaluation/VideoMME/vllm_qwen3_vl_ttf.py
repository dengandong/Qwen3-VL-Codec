"""Temporal Token Fusion utilities and vLLM Qwen3-VL patch hooks.

The pure PyTorch TTF implementation in this file is independent of vLLM so it
can be tested on CPU.  The vLLM integration intentionally refuses to run the
raw-video threshold path on local Qwen3-VL/vLLM builds that cannot know the
post-ViT retained length before prompt expansion.  That fail-fast behavior is
preferable to dense-placeholder pseudo-pruning, which would not reduce LLM KV.
"""
...
def _validate_params(
    ...
    if anchor not in {"auto", "first", "last"}:
        raise ValueError(f"Unsupported TTF anchor={anchor!r}; expected auto/first/last")
...
def _target_retained_count(total_tokens: int, anchor_tokens: int, retain_ratio: float) -> int:
    ...
    return max(int(anchor_tokens), min(int(total_tokens), target))


def _select_anchor(features: torch.Tensor, anchor: str) -> int:
    ...
    global_mean = F.normalize(global_mean, dim=-1, eps=1e-6)
    sim = (frame_mean * global_mean).sum(dim=-1)
...
def build_ttf_plan(
    ...
    *,
    ...
    """Build a TTF selection plan for one video.

    Args:
        features: Projector/merger output shaped ``[T, H, W, C]``.
        threshold: Source tokens with best local-anchor cosine similarity
            greater than or equal to this value are removed.
        window_radius: Local spatial search radius around the same row/column.
        anchor: ``auto``, ``first`` or ``last``.
        order: ``paper`` puts the anchor frame first, then source frames in
            temporal order. ``temporal`` keeps original time order.
        disable_fusion: Internal testing/baseline switch that preserves dense
            temporal row-major order.
    """
    _validate_params(threshold, window_radius, anchor, order, budget_mode, retain_ratio)
    if features.ndim != 4:
    ...
    total = t * h * w
    anchor_idx = _select_anchor(features, anchor)
    ...
    anchor_tokens = normalized[anchor_idx]
    ...
    source_frames = [idx for idx in range(t) if idx != anchor_idx]
    source = normalized[source_frames]  # [S, N, C]
    sim = torch.einsum("snc,nkc->snk", source, anchor_candidates)
    sim = sim.masked_fill(~cand_unique.unsqueeze(0), -torch.inf)
    best_sim, best_k = sim.max(dim=-1)
    matched = (
        cand_idx.unsqueeze(0)
        .expand(len(source_frames), -1, -1)
        .gather(2, best_k.unsqueeze(-1))
        .squeeze(-1)
    )

    effective_threshold = None
    if budget_mode == "threshold":
        keep_source = best_sim < float(threshold)
        for local_idx, frame_idx in enumerate(source_frames):
            keep_mask[frame_idx] = keep_source[local_idx]
    else:
        target = _target_retained_count(total, tokens_per_frame, float(retain_ratio))
        source_budget = target - tokens_per_frame
        if source_budget >= len(source_frames) * tokens_per_frame:
            for frame_idx in source_frames:
                keep_mask[frame_idx] = True
        elif source_budget > 0:
            score = best_sim.reshape(-1)
            order_idx = torch.argsort(score, stable=True)
            selected_source = order_idx[:source_budget]
            effective_threshold = float(score[order_idx[source_budget - 1]].item())
            frame_local = selected_source // tokens_per_frame
            token_local = selected_source % tokens_per_frame
            for local_idx, frame_idx in enumerate(source_frames):
                keep_mask[frame_idx, token_local[frame_local == local_idx]] = True
        else:
            effective_threshold = None

    best_full = torch.empty((t, tokens_per_frame), device=device, dtype=torch.float32)
    best_full.fill_(float("nan"))
    matched_full = torch.full((t, tokens_per_frame), -1, device=device, dtype=torch.long)
    best_full[anchor_idx] = 1.0
    matched_full[anchor_idx] = torch.arange(tokens_per_frame, device=device, dtype=torch.long)
    for local_idx, frame_idx in enumerate(source_frames):
        best_full[frame_idx] = best_sim[local_idx]
        matched_full[frame_idx] = matched[local_idx]

    if order == "paper":
        frame_order = [anchor_idx] + [idx for idx in range(t) if idx != anchor_idx]
    else:
    ...
    if output_chunks:
        ...
        # This should be unreachable because the anchor frame is always kept.
...
def _compress_video_outputs(
    ...
    for video_idx, row in enumerate(grid_thw):
        ...
        if _verbose() or debug_verify:
            ...
            print(
                f"[TTF-vLLM] video[{video_idx}] anchor={plan.anchor_idx} order={order} "
                f"budget_mode={budget_mode} tokens={plan.original_token_count}->{plan.retained_token_count} "
...
def _enabled_mode() -> str:
    return os.environ.get("QWEN3VL_TTF_MODE", "none").strip().lower()


def _threshold() -> float:
    return float(os.environ.get("QWEN3VL_TTF_THRESHOLD", "0.70"))


def _budget_mode() -> str:
    return os.environ.get("QWEN3VL_TTF_BUDGET_MODE", "retain_ratio").strip().lower()


def _retain_ratio() -> float:
    return float(os.environ.get("QWEN3VL_TTF_RETAIN_RATIO", "0.25"))


def _window_radius() -> int:
    return int(os.environ.get("QWEN3VL_TTF_WINDOW_RADIUS", "1"))


def _anchor() -> str:
    return os.environ.get("QWEN3VL_TTF_ANCHOR", "auto").strip().lower()


def _order() -> str:
    return os.environ.get("QWEN3VL_TTF_ORDER", "paper").strip().lower()


def _debug_verify() -> bool:
    return os.environ.get("QWEN3VL_TTF_DEBUG_VERIFY", "0") == "1"


def _verbose() -> bool:
    return os.environ.get("QWEN3VL_TTF_QUIET", "0") != "1"


def _identity_only() -> bool:
    return os.environ.get("QWEN3VL_TTF_DISABLE_FUSION", "0") == "1" or (
...
def _unsupported_dynamic_message() -> str:
    return (
        "[TTF-vLLM] Current local vLLM Qwen3-VL API expands video placeholders "
        ...
        "(QWEN3VL_TTF_DISABLE_FUSION=1 or threshold > 1) for parity checks, or "
...
def apply_patch(
    ...
    os.environ["QWEN3VL_TTF_THRESHOLD"] = str(float(threshold))
    os.environ["QWEN3VL_TTF_BUDGET_MODE"] = budget_mode
    os.environ["QWEN3VL_TTF_RETAIN_RATIO"] = str(float(retain_ratio))
    os.environ["QWEN3VL_TTF_WINDOW_RADIUS"] = str(int(window_radius))
    os.environ["QWEN3VL_TTF_ANCHOR"] = anchor
    os.environ["QWEN3VL_TTF_ORDER"] = order
    os.environ["QWEN3VL_TTF_DEBUG_VERIFY"] = "1" if debug_verify else "0"
    ...
    for name in (
        ...
        if not required.get(name, False):
            raise AttributeError(f"[TTF-vLLM] Unsupported local Qwen3-VL API; missing {name}")
    ...
    def patched_process_image_input(self, image_input):
        ...
        if image_input["type"] == "image_embeds":
            ...
            if self.use_data_parallel:
                raise RuntimeError("TTF vLLM patch does not support mm_encoder_tp_mode=data yet.")
    ...
    def patched_process_video_input(self, video_input):
        ...
        if video_input["type"] == "video_embeds":
            raise NotImplementedError("[TTF-vLLM] budgeted TTF currently requires raw pixel_values_videos.")
        ...
        if _debug_verify():
            for idx, (emb, pos) in enumerate(zip(mm_embeddings_out, mm_positions)):
                if emb.shape[0] != pos.shape[1]:
                    raise RuntimeError(
                        f"[TTF-vLLM] recompute invariant failed for mm[{idx}]: "
    ...
    if _verbose():
        print(
            f"[TTF-vLLM] enabled mode={mode} budget_mode={budget_mode} "
            f"retain_ratio={float(retain_ratio):.4f} threshold={float(threshold):.4f} "
            f"radius={int(window_radius)} anchor={anchor} order={order}"
        )
        print(f"[TTF-vLLM] local_api={required}")
